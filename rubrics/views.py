...
def type_stay(request,slug):
    ...
    if request.LANGUAGE_CODE == 'ru':
        ...
        stay_count_gostdom = stay_list.filter(type__name='Гостевой дом').count()
        stay_count_zona = stay_list.filter(type__name='Зона отдыха').count()
    elif request.LANGUAGE_CODE == 'en':
        stay_count_otel = stay_list.filter(type__name='Otel').count()
        stay_count_sanat = stay_list.filter(type__name='Sanatorium').count()
        stay_count_gostdom = stay_list.filter(type__name='Guest houses').count()
        stay_count_zona = stay_list.filter(type__name='Rezt zonez').count()
    else:
        stay_count_otel = stay_list.filter(type__name='Отель').count()
        stay_count_sanat = stay_list.filter(type__name='Санаторий').count()
        stay_count_gostdom = stay_list.filter(type__name='Қонақ үй').count()
